...
class Inventory_model extends CI_Model{
	...
	function get_store_inventory(){
		...
		$this->db->select('id, name, buyPrice, sellPrice, quantity, unit');
		return($this->db->get('resource'));
	}

	function get_sell_inventory($family_name){
		$query = $this->db->query("SELECT resource.name as name, resource.sellPrice as sellPrice, resource.buyPrice as buyPrice, resource.id as id, resource.category as category, resource.unit as unit, inventory.quantity as quantity 
					 FROM resource
